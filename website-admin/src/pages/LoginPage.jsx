...
const Login = () => {
    ...
    const handleLogin = async () => {
        ...
        try {
            const response = await fetch(`${API_BACKEND}/admin_login`, {
                method: 'POST',
            ...
            if (response.ok) {
                ...
                setName('');
                setPassword('');
                navigate('/dashboard', { state: { display_name: admin_name } });
            } else {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4 sm:p-8">
            <div className="bg-white p-6 rounded-lg shadow-md w-full max-w-md flex flex-col gap-4">
                <h2 className="text-center font-bold text-2xl text-gray-800">Login</h2>
                <p className="text-center text-gray-600 text-sm">Login to your account</p>
